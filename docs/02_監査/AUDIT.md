...
8. Market側に `internal` / `internal-only` が表示されていない。
9. `admin.html` は本番操作不可のモックとして動作している。
10. GitHub Pagesへ置いてはいけない情報が混ざっていない。
11. GitHub Pages公開URLで変更が反映されている。

## 推奨コマンド

```bash
node -e "JSON.parse(require('fs').readFileSync('data/contents.json','utf8')); console.log('contents.json OK')"
node -c assets/js/adminMockPage.js
git diff --check
```
