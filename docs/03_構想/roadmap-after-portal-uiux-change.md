...
- v0.16は限定範囲で完了扱い。
- `portal_state` の甲乙分離、`works_portal` app_instance自動付与方針、SeatFlow表示中レイアウトの限定的な保存・読込確認までを対象とした。
- SeatFlow複数レイアウト、名簿、QR、NFC、メモ、履歴、競合解決、同時編集を含む完全クラウド同期はv0.16外のPARKEDタスクとする。
...
v0.14.12、v0.15、v0.16は完了扱い。現在はv0.17 バックアップ・復元へ進む前提が整っている。
...
- v0.16ではSupabase保存対象のRLSと他社データ混入テストに集中し、SeatFlow完全クラウド同期はPARKEDへ移した
- v0.17ではバックアップ・復元の範囲を整理する
