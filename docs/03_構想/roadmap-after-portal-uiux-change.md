...
- v1.2 企業情報編集。
- v1.3 アプリ追加申請。
  - v1.3a アプリ追加申請UI mock: 完了。DB保存なし、画面内状態のみ。
  - v1.3b アプリ追加申請DB保存設計: 完了。`app_add_requests` 専用テーブル案、RLS方針、status案、運営側フローを整理。
  - v1.3c app_add_requests migration / RLS実装: 次候補。実DB適用は人間確認前提。
...
v0.14.12、v0.15、v0.16、v0.17a、v0.18、v1.0、v1.1b、v1.2、v1.2b、v1.3a、v1.3bは完了扱い。v0.17b以降はmain未反映のためPARKEDに戻したまま、次は v1.3c app_add_requests migration / RLS実装へ進む。

決済・購入履歴は、実アプリの商品化方針、販売用UI/UX、購入ページ、販売前QAが固まった後の v1.8 で扱う。独自ドメイン公開準備、レンタルサーバー移行はロードマップに追加しない。
...
- v1.3aでは、`account.html` 上でDB保存なしのアプリ追加申請UI mockを確認した
- v1.3bでは、申請専用テーブル `app_add_requests` のDB保存設計を整理した
