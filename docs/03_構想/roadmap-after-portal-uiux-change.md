...
- v1.1 簡易管理者画面: まず方針整理。次に進むなら実DBに接続しないmock簡易管理者画面。
- v1.2 企業情報編集。
- v1.3 アプリ追加申請。
- v1.4 購入ページ。
- v1.5 決済・購入履歴。
...
v0.14.12、v0.15、v0.16、v0.17a、v0.18、v1.0は完了扱い。v0.17b以降はmain未反映のためPARKEDに戻したまま、次は v1.1 簡易管理者画面へ進む。
...
- v1.1では、いきなり実装せず、簡易管理者画面の対象・権限・DB/RLS影響を整理する
- 本物の企業横断管理画面は、管理者権限設計、RLS policy、管理者専用APIなどが必要なため今すぐは非推奨
- 次に進むなら v1.1b mock簡易管理者画面で、実DB接続なし・企業横断データ取得なし・DB/RLS変更なしを前提にする
