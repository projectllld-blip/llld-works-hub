...
- v1.6 購入ページ。
  ...
  - 確認後は `sessionStorage` によるポータル一時追加に留め、購入履歴や正式な `app_instances` 反映はまだ行わない。
- v1.7 購入後の利用開始・利用中アプリ反映。
  - 確認後に正式な利用中アプリへ反映するためのDB / RLS / 状態管理を整理する。
- v1.8a 商品・料金管理 方針整理。
  - 商品名、料金形態、価格、CTA、掲載状態、任意サポートをどこで管理するか整理する。
  - 本物管理者画面、商品マスタ、料金マスタ、管理者権限、変更履歴、RLS / 管理者APIの要否を判断する。
- v1.8b 商品マスタ / 料金マスタ DB設計。
  - 決済前に、価格とCTAの正本を静的JSONからDBへ移すか検討する。
- v1.8c 管理者画面 商品・料金mock。
  - 実DB接続前に、運営が商品・料金・CTAをどう確認するかmockで整理する。
- v1.8 販売前QA・導入テスト。
- v1.9 決済・購入履歴。
- v2.0 β販売開始。
- v2.1 小規模事業者向け販売版。
...
v0.14.12、v0.15、v0.16、v0.17a、v0.18、v1.0、v1.1b、v1.2、v1.2b、v1.3、v1.3f、v1.4、v1.5は完了扱い。v0.17b以降のバックアップ実装、SeatFlow完全クラウド同期、購入者向け `app_add_requests` 利用申請はPARKEDに戻したまま、現在は v1.6 購入ページへ進む。

決済・購入履歴は、実アプリの商品化方針、販売用UI/UX、購入ページ、購入後の利用開始・利用中アプリ反映、商品・料金管理方針、販売前QAが固まった後の v1.9 で扱う。独自ドメイン公開準備、レンタルサーバー移行はロードマップに追加しない。
