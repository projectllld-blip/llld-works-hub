...
- v1.7d 無料 / β版 app_instances反映MVP。
  - 状態: 実装済み / 人間確認待ち。
  - `pdf-tool -> pdf_tool -> active`、`quiz-maker -> quiz_maker -> trial` の2件だけをallowlist対象にして `app_instances` へ正式反映する。
  - `dakokun` / `seatflow` は副CTAにβ版があるが、正本分類が `consultation` / `inquiry-only` のため初期allowlist対象外。
  - 有料 / サブスク / 準備中 / 開発相談 / 社内限定は `app_instances` にも `sessionStorage` にも追加しない。
  - 既存 `active` / `trial` は重複insertせず、`paused` はallowlist対象のみ復帰、`disabled` はユーザー操作で復活させない。
  - 完了扱いには、Supabase Dashboardでの `app_instances` 反映確認とブラウザ確認が必要。
...
v0.14.12、v0.15、v0.16、v0.17a、v0.18、v1.0、v1.1b、v1.2、v1.2b、v1.3、v1.3f、v1.4、v1.5、v1.6、v1.6b、v1.6c、v1.7方針整理、v1.7b、v1.7c、v1.7d事前確認は完了扱い。v1.7d 無料 / β版 app_instances反映MVPは実装済み / 人間確認待ち。v0.17b以降のバックアップ実装、SeatFlow完全クラウド同期、購入者向け `app_add_requests` 利用申請はPARKEDに戻したまま、v1.7d確認後に v1.7e account / portal / my-apps 表示整合へ進む。
