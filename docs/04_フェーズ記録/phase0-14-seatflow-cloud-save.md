...
v0.16追加修正では、`seat_layout` のJSONを現在表示中の1レイアウトだけでなく、SeatFlow全体状態へ拡張した。

保存対象:

- 複数レイアウト一覧
- 各レイアウトID / 名前 / floor / outline / items
- 選択中レイアウトID
- 表示設定の一部
- `schemaVersion`
- `revision`
...
- クラウド読込完了後のユーザー操作だけをdebounceして保存する。
- クラウド読込前、またはlocalStorageを表示しただけの状態では自動保存しない。
- 手動の `クラウド保存` は保険として残す。

衝突検知:

- 保存前にクラウド側の `revision` を確認する。
- 別タブまたは別端末で新しいrevisionがある場合、自動保存せず、`クラウド再読込` または `この内容で上書き保存` を選べるようにする。
