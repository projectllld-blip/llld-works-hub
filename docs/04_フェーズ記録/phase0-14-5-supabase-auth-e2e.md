...
2026-06-26時点では、`data/site-config.json` をSupabase modeへ変更済み。

接続先:

```text
supabaseUrl: https://pjcyusmifjoxxuqwgaoh.supabase.co
supabaseAnonKey: sb_publishable...uz9F
```
...
- Supabase Auth settings endpointに接続できた。
- `signup` APIは200で応答した。
- Auth user相当のレスポンスが返った。
...
signup後にsession / access tokenが返らず、password loginでも以下の状態になった。
...
そのため、RLS付きの `company_accounts` / `app_instances` / `app_data` 確認には進んでいない。
...
Supabase Dashboardで検証用に以下のどちらかを行う。

1. テストメールの確認リンクを開いてメール確認を完了する。
2. 検証中のみ Email confirmation をOFFにする。
