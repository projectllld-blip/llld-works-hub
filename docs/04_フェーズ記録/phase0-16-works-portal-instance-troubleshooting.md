...
## 2026-07-01 migration適用後確認

人間側で `supabase/migrations/20260627_v016_ensure_works_portal_app_instance.sql` をSupabase SQL Editorへ適用済み。

確認済み:

- `app_instances(company_account_id, app_key)` の重複チェック結果: 問題なし。
- 既存 `company_accounts` すべてに `works_portal` app_instance がある。
- `works_portal` app_instance に `portal_state` app_data がある。
- 新規signup時に `selected_app_keys` に関係なく `works_portal` app_instance が作られる。
- 乙アカウントで `portal.html` の「ポータル保存がまだ有効化されていません」が消えた。
- 乙アカウントで `portal_state` を保存・復元できた。
- RLSは無効化していない。
- service role keyは使っていない。

判断:

- `works_portal` app_instance自動付与の問題は解消済み。
- `portal_state` のRLS確認を再開できる。
- v0.16全体としては、残りの `seat_layout` 甲乙分離確認が完了するまで完了扱いにしない。
