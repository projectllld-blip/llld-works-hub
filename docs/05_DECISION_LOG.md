...
## 2026-07-01 v0.18を検証環境確認として扱う

- 決定内容: v0.18 検証環境デプロイは販売開始や本番運用ではなく、GitHub Pages公開版がv1.0前の検証環境として使えるか確認するPhaseとして扱う。CodexはHTTP 200、`site-config`、secret混入なし、PARKED範囲の整合性まで確認し、signup / login / 保存 / RLS / 実機表示 / v1.0へ進む判断は人間確認に戻す。
- 理由: ブラウザ操作、Supabase Auth URL Configuration、RLS最終確認、iPad / スマホ実機確認はCodexだけで完了判断できず、誤ってv1.0へ進むと本番相当の判断を自動化してしまうため。
- 影響範囲: `docs/04_フェーズ記録/phase0-18-verification-deploy.md`、`docs/00_PROJECT_STATUS.md`、`docs/06_TASK_QUEUE.md`、v1.0開始判断。
- 関連Phase: v0.18、v1.0。
- 取り消し条件: 人間が公開版ブラウザ確認、Supabase設定確認、RLS最終確認を完了し、v1.0へ進む判断をした場合。
