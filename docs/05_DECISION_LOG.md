...
## 2026-07-02 v1.5販売UIでは相談導線を通常商品カードから分離する

- 決定内容: Works Marketの通常商品一覧は、無料 / β版 / 有料化予定 / 準備中の商品を中心にし、`priceType = consultation`、`saleStatus = inquiry-only`、`primaryCtaType = consultation` のコンテンツは `導入相談・カスタマイズ相談` セクションへ分離する。商品詳細では、提供状態、状態メモ、今押せるCTAを表示する。
- 理由: 相談型コンテンツを通常商品カードに混ぜると、購入者が「相談しないとアプリを使えない」と誤解しやすいため。v1.3 / v1.4で決めた、購入 / 利用開始 / 任意サポートの分離を販売画面に反映するため。
- 影響範囲: `marketplace.html`、`content-detail.html`、`assets/js/marketPages.js`、`assets/css/style.css`、`docs/04_フェーズ記録/phase1-5-sales-uiux.md`。
- 関連Phase: v1.5、v1.6、v1.7、v1.8、v1.9。
- 取り消し条件: 人間判断により、相談型コンテンツを通常の商品カードへ戻す、または購入ページ / 決済実装を先行する方針に変更する場合。
