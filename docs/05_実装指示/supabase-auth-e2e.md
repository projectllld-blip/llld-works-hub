...
- migration / seedが適用済みでも、Email confirmationがONの場合はsignup直後にsessionが返らない。
- sessionがない状態ではRLS付きの `company_accounts` / `app_instances` / `app_data` を正しく確認できない。
- `email_not_confirmed` が返った場合は、メール確認完了後にloginから再開する。
