...
v0.17では、企業アカウント単位の最低限バックアップ方針を整える。

初回MVP候補は、ログイン中企業アカウントが自社データをブラウザからJSONエクスポートする方式。
...
- 復元先はログイン中企業アカウントへ紐づけ直す。
- `app_instance_id` は信用せず、ログイン中企業の `app_instances.app_key` から引き直す。
- 復元前に現在データを自動バックアップする。
- まずは `portal_state` など限定範囲から検討する。
