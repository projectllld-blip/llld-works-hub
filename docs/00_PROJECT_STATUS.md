...
v1.3c app_add_requests migration / RLS実装案では、`supabase/migrations/20260701_v13c_app_add_requests.sql` を追加した。実DBには未適用。人間がSupabase Dashboard / SQL Editorで内容確認・適用・RLS確認を行う必要がある。
...
- 実DBへは未適用。v1.3dへ進む前に、人間がSupabase Dashboard / SQL Editorで確認・適用・RLS確認する必要がある。
