...
## 現在の最新フェーズ
v0.16 RLS・他社データ混入テスト中。

LLLD Works Hub / Works Market は、社内ポータル・販売マーケット・小型業務アプリ群を統合するプロジェクトです。

現在は、A0.1〜A0.5 Codex半自動運用の初期実用ラインが完了し、本線 v0.16 RLS・他社データ混入テストを進めています。`portal_state` の甲乙分離は確認済みで、`seat_layout` の甲乙分離確認が残っています。
...
## 一時停止中の本線
- `portal.html` の新規機能開発。ただし次の本線候補は v0.16 RLS・他社データ混入テスト。
- `marketplace.html` の新規機能開発。ただし次の本線候補は v0.16 RLS・他社データ混入テスト。
- 本体HTMLアプリの機能追加
...
A0.6 / A0.7へ進むかは、人間確認後に判断する。現時点の次候補は本線 v0.16 RLS・他社データ混入テスト。
