...
## 現在の最新フェーズ
A0.1〜A0.5 Codex半自動運用の初期実用ライン完了。

LLLD Works Hub / Works Market は、社内ポータル・販売マーケット・小型業務アプリ群を統合するプロジェクトです。

現在は、ポータルサイトとマーケット本体の開発を一旦停止し、Codex半自動運用基盤づくりを優先します。
...
## 一時停止中の本線
- `portal.html` の機能開発。ただし次の本線候補は v0.15 エラー処理・空状態。
- `marketplace.html` の機能開発。ただし次の本線候補は v0.15 エラー処理・空状態。
- 本体HTMLアプリの機能追加
...
A0.6 / A0.7へ進むかは、人間確認後に判断する。現時点の次候補は本線 v0.15 エラー処理・空状態。
