...
現在の作業対象は v1.6 購入ページ。決済実装には入らず、`purchase-confirm.html` で購入 / 利用開始確認、任意サポート選択、ポータルへの一時追加導線を整理している。あわせて、購入ページ直後の `v1.6b 商品・料金・CTA設定の整理`、`v1.6c 管理者mock画面：商品/料金/表示状態確認` をロードマップに追加し、本物管理者画面での料金・掲載・購入状態編集は v2.x 以降へ分離する。
