...
## 現在の最新フェーズ
v0.17 バックアップ・復元 方針整理中。

LLLD Works Hub / Works Market は、社内ポータル・販売マーケット・小型業務アプリ群を統合するプロジェクトです。

現在は、A0.1〜A0.5 Codex半自動運用の初期実用ラインが完了し、本線 v0.17 バックアップ・復元の対象・方式・復元リスクを整理しています。
...
## 進行中の本線
- v0.17 バックアップ・復元: 方針整理中。初回は実装せず、ログイン中企業アカウントの自社 `app_data` JSONエクスポートをMVP候補とする。
