...
v1.1では、いきなり本物の企業横断管理画面を実装せず、まず管理画面で見る項目、見ない項目、DB / RLS / 管理者権限の扱いを整理する。

LLLD Works Hub / Works Market は、社内ポータル・販売マーケット・小型業務アプリ群を統合するプロジェクトです。

現在は、A0.1〜A0.5 Codex半自動運用の初期実用ラインと、本線 v1.0 アカウント別クラウド基盤MVP完成まで完了しています。次は v1.1 簡易管理者画面です。
...
## 一時停止中の本線
- `portal.html` の新規機能開発。ただし次の本線候補は v1.1 簡易管理者画面。
- `marketplace.html` の新規機能開発。ただし次の本線候補は v1.1 簡易管理者画面。
- 本体HTMLアプリの機能追加
...
A0.6 / A0.7へ進むかは、人間確認後に判断する。現時点の次候補は本線 v1.1 簡易管理者画面。
...
## v1.1開始前の注意
- v1.1 簡易管理者画面は、まず方針整理から始める。
- 管理者画面で何を見るか、企業一覧が必要か、`app_instances` や `portal_state` 状態確認が必要かを整理する。
- 管理者認証・管理者権限・企業横断データ閲覧は人間判断が必要。
- DB / RLS / migration変更が必要な場合は、案だけ作って `HUMAN_REQUIRED: YES` で止まる。
- service_role keyをフロントへ入れない。
- RLSを無効化しない。

## v1.1方針整理メモ
- 管理者画面で見たいものは、企業一覧、企業名、登録日時、最終更新日時、利用アプリ数、`app_instances` 一覧、`portal_state` の有無、`app_data` 件数、異常状態。
- v1.1では `app_data` 本文、`portal_state` 本文、個人情報、Auth user詳細、決済情報、SeatFlow詳細データ、他社の実データ内容は見ない。
- 現在のRLSで企業横断一覧が取得できないことは正常。
- 本物の企業横断管理画面には、管理者権限設計、RLS policy、管理者専用APIなどの検討が必要。
- 現時点の推奨は v1.1a 方針整理のみ。次に進むなら実DBに接続しない v1.1b mock簡易管理者画面。
