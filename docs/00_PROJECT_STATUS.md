...
現在は、A0.1〜A0.5 Codex半自動運用の初期実用ラインが完了し、本線 v0.17 バックアップ・復元のうち、バックアップJSONエクスポート、DB更新なしの読込・検証・プレビュー、`portal_state` 限定復元設計まで完了しています。
...
## 進行中の本線
- v0.17 バックアップ・復元: 復元は未実装。次は `portal_state` 限定復元MVPへ進むか人間判断する。
