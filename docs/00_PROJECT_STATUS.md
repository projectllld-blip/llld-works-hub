...
v1.3c app_add_requests migration / RLS実装案では、`supabase/migrations/20260701_v13c_app_add_requests.sql` を追加した。実DBには未適用。環境分離状況確認の結果、GitHub Pages公開版も現在のSupabase projectを参照しているため、2026-07-01の人間判断で「現在のSupabase project = 本番相当DB」「新しく作るSupabase project = 検証用DB」と確定した。現行Supabase projectへのSQL適用は本番相当DB変更として必ず `HUMAN_REQUIRED: YES` とし、人間がSQL全文確認後に手動適用する。
