...
現在の作業対象は v1.6 購入ページ。決済実装には入らず、`purchase-confirm.html` で購入 / 利用開始確認、任意サポート選択、無料 / β版だけのポータル一時追加導線を整理している。有料 / サブスクは購入確認のみで、決済未実装中はポータルや利用中アプリへ追加しない。あわせて、購入ページ直後の `v1.6b 商品・料金・CTA設定の整理`、`v1.6c 管理者mock画面：商品/料金/表示状態確認` をロードマップに追加し、本物管理者画面での料金・掲載・購入状態編集は v2.x 以降へ分離する。
...
## 進行中の本線
- v1.6 購入ページ: `purchase-confirm.html` を無料 / β版 / 買い切り / サブスクの確認ページとして整える。無料 / β版は確認後に `sessionStorage` で `portal.html` に一時追加するMVP。有料 / サブスクは購入確認のみで、決済未実装中はポータルや利用中アプリへ追加しない。正式な決済、購入履歴、`app_instances` 反映は後続フェーズ。
- 商品・料金・CTA設定ロードマップ整理: 商品名、料金形態、価格、CTA、掲載状態、任意サポートなどは、まず v1.6b / v1.6c で静的データ寄せと固定mock確認を行う。本物管理者画面 / 商品マスタ / 料金マスタ / 管理者DB編集は v2.x 以降に分離する。
...
- v1.6b 商品・料金・CTA設定の整理: 購入ページ、マーケットページ、アプリカードで使う商品情報を静的JSONまたは既存データ構造に寄せ、コード直書きを減らす。
- v1.6c 管理者mock画面：商品/料金/表示状態確認: 実DB接続なしで、商品一覧、価格、表示状態、CTA、サポート設定、購入後の反映先 `app_key`、異常状態を固定mock確認する。
