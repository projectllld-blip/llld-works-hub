...
現在はv1.3b アプリ追加申請DB保存設計と、A0.8/A0.9 docs-only自動マージ判定の厳密化まで完了。A0.9 自動マージセットアップ不足調査では、GitHub側の `Allow auto-merge` 設定確認が必要と判定し、その後人間側でONにしたため、次のsafe docs-only PRで `AUTO_MERGE_STATUS: enabled` になるか継続確認する。次の本線候補は v1.3c app_add_requests migration / RLS実装。
