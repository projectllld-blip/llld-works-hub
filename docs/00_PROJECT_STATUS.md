...
## 現在の最新フェーズ
v1.4 実アプリの商品化方針整理まで完了。

現在の作業対象は v1.5 販売用UI/UX強化。

GitHub Pages公開版、認証・ポータル導線、PC / iPad / スマホ表示の大きな崩れがないことを人間確認済み。

v1.0は販売開始ではなく、企業アカウント単位のクラウド基盤MVP完成宣言。

v1.4 実アプリの商品化方針整理では、既存アプリやコンテンツを「無料」「有料化予定」「β版」「準備中」「相談導線」「PARKED」に分類し、購入ページや決済より前に商品としてどう見せるかを整理した。

現在はv1.5 販売用UI/UX強化として、Works Marketの一覧と商品詳細に、商品状態、CTA、相談導線分離、PARKED非表示の方針を反映している。
...
- v1.4 実アプリの商品化方針整理: 商品状態を「無料」「有料化予定」「β版」「準備中」「相談導線」「PARKED」に分類し、v1.5 / v1.6へ進むための表示方針を整理。
...
## 進行中の本線
- v1.5 販売用UI/UX強化: `marketplace.html` / `content-detail.html` の状態表示、CTA、相談導線分離を整える。決済、DB更新、利用開始処理は含めない。
...
## v1.5 販売用UI/UX強化
- `marketplace.html` の状態ガイドを、無料 / β版 / 有料化予定 / 準備中が分かる表示へ寄せる。
- 通常の商品一覧は、相談導線を除いた無料 / β版 / 有料化予定 / 準備中の商品に限定する。
- `priceType = consultation`、`saleStatus = inquiry-only`、`primaryCtaType = consultation` は `導入相談・カスタマイズ相談` セクションへ分離する。
- 商品カードと商品詳細では、提供状態、状態メモ、今押せるCTAを表示する。
- 決済、購入履歴、`app_instances` 追加、利用開始DB処理、Supabase migration、RLS変更は行わない。
