...
現在はv1.2 企業情報編集の方針整理と、A0.7 HUMAN_REQUIRED判定修正・安全PR自動マージ実装まで完了。次の本線候補は v1.2b 企業情報編集フォームMVP。
...
- v1.2 企業情報編集 方針整理: `account.html` 内で自社 `company_accounts` の基本情報を編集する方針を整理。実装は未着手。
- A0.1 Codex半自動運用基盤
...
- docs / migration上は `company_accounts_update_own` があるが、v1.2b実装前または実装後に実DBでupdate policyが有効か人間確認する。
- DB / RLS / migration変更が必要なら `HUMAN_REQUIRED: YES` で止まる。
