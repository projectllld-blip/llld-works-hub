...
## 現在の最新フェーズ
v0.17c バックアップJSON読込・検証・プレビューまで実装済み。

LLLD Works Hub / Works Market は、社内ポータル・販売マーケット・小型業務アプリ群を統合するプロジェクトです。

現在は、A0.1〜A0.5 Codex半自動運用の初期実用ラインが完了し、本線 v0.17 バックアップ・復元のうち、バックアップJSONエクスポートと、DB更新なしの読込・検証・プレビューまで実装しています。
...
## 進行中の本線
- v0.17 バックアップ・復元: 復元は未実装。次は限定復元設計を検討する。
