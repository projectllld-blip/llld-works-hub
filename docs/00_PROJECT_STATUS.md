...
- v1.3c app_add_requests migration / RLS実装案: `app_add_requests` 作成migration案と、自社申請のみselect / insertできるRLS policy案を作成。現行Supabase projectへ人間が手動適用済み。
...
- v1.3d account.html 申請DB保存の人間ブラウザ確認 / Supabase Dashboard確認
- v1.3e 管理者側 申請確認MVP
- v1.4 実アプリの商品化方針整理
- v1.5 販売用UI/UX強化
- v1.6 購入ページ
...
- v1.3dでは、`account.html` から `app_add_requests` への申請保存、申請済み表示の復元を実装する。

## v1.3d account.html 申請DB保存
- `account.html` のアプリ追加申請UIをSupabase modeで `app_add_requests` へ保存する。
- 保存済み申請をselectし、対象アプリを `申請済み` 表示にする。
- mock modeではDBへ接続せず画面内状態だけで確認する。
- 申請作成だけでは `app_instances` を追加しない。
- `app_data`、`company_accounts`、`plan_status` は変更しない。
- 人間ブラウザ確認とSupabase Dashboard確認が必要。
