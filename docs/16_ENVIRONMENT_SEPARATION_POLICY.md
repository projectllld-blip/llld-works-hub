...
今後このmigrationを実行する場合は、本番相当DBへのschema変更として扱い、必ず `HUMAN_REQUIRED: YES` とする。

## v1.3c再開条件

v1.3c `app_add_requests` migrationを再開するには、以下を満たすこと。

1. 現在のSupabase projectを本番相当DBとして扱うか、人間が判断している。
2. または、検証用Supabase projectを別途作成し、そちらに適用する方針が決まっている。
3. 適用先project名を人間が明示している。
4. migration SQLを人間が全文確認している。
5. `drop`、`truncate`、`delete`、`disable row level security` がないことを確認している。
6. 既存テーブルや既存データへ破壊的影響がないことを確認している。
7. SQL Editorでの実行は人間が行う。
8. 適用後、RLSが有効なままか人間が確認する。
...
- 検証用projectを作るまでは、migration適用を本番相当DB変更として扱う。
