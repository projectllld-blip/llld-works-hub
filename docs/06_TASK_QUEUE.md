...
- v1.3d account.html 申請DB保存の人間確認
- v1.3e 管理者側 申請確認MVP
- v1.4 実アプリの商品化方針整理
- v1.5 販売用UI/UX強化
- v1.6 購入ページ
...
- v1.3dでは、`account.html` のアプリ追加申請UIを `app_add_requests` 保存へ接続する。`app_instances` 自動追加、`app_data` 保存、`company_accounts` / `plan_status` 更新は行わない。
