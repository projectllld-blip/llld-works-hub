...
11. 外部リンク、内部の `/apps/` 配下のHTMLを問わず、コンテンツを開く操作は新しいタブで開いてください。
12. 新しいタブで開くリンクには `target="_blank"` と `rel="noopener"` を付け、JavaScript経由の場合も `window.open(url, "_blank", "noopener")` を使ってください。
...
- お気に入り：カードのお気に入りボタンを押すと左ツールバーの「お気に入り」欄に表示
